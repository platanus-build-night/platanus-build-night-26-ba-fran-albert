...
interface EHRPaginatedResponse<T> {
  data: T[];
  total: number;
  page: number;
  limit: number;
...
async function fetchLabResults(
  ...
  const studiesRaw = await ehrFetch<unknown>(
    buildUrl(studiesEndpoint, { id: patientId }, ehrUrl),
    token,
  );
  const studies = ensureArray<EHRStudyDTO>(studiesRaw);
  ...
  const labDataRaw = await ehrFetch<unknown>(labDataUrl, token);
  const labData = ensureArray<EHRLabDataDTO>(labDataRaw);
...
export async function searchPatientsEHR(
  ...
  const url = `${getBaseUrl(ehrUrl)}${endpoint}?search=${encodeURIComponent(query)}`;
  const response = await ehrFetch<EHRPaginatedResponse<EHRSearchResultDTO> | EHRSearchResultDTO[]>(url, token);
...
function mapPatient(dto: EHRPatientDTO): Patient {
  ...
  return {
    ...
    documentNumber: dto.document ?? dto.userName ?? "",
...
function mapEvoluciones(dtos: EHREvolucionDTO[]): Evolution[] {
  return dtos.map((dto) => {
    const dateStr = dto.date ?? dto.createdAt ?? "";
    const doctorFirst = dto.doctor?.firstName ?? dto.doctor?.name ?? "";
    return {
      id: dto.id,
      doctorName: dto.doctor
        ? `Dr. ${doctorFirst} ${dto.doctor.lastName}`
        : "Sin datos",
      specialty: dto.doctor?.specialities?.[0]?.name ?? "General",
      date: dateStr.split("T")[0],
      fields: (dto.data ?? []).map((d) => ({ name: d.fieldName ?? d.name ?? "", value: d.value })),
    };
  });
}
...
function mapLabs(
  ...
  const studyDateMap = new Map(studies.map((s) => [s.id, (s.date ?? s.createdAt ?? "").split("T")[0]]));
...
/** Normalise API responses that may be arrays, paginated objects, or wrapped with a named key. */
function ensureArray<T>(data: unknown): T[] {
  ...
  if (data && typeof data === "object") {
    ...
    // { antecedentes: [...] } or { evoluciones: [...] } — named wrapper
...
export async function fetchPatientRecord(
  ...
  const antecedentes =
    antecResult.status === "fulfilled"
      ? mapAntecedentes(ensureArray<EHRAntecedentesDTO>(antecResult.value))
      : [];

  const evolutions =
    evoResult.status === "fulfilled"
      ? mapEvoluciones(ensureArray<EHREvolucionDTO>(evoResult.value))
      : [];

  const medications =
    medsResult.status === "fulfilled"
      ? mapMedications(ensureArray<EHRMedicationDTO>(medsResult.value))
      : [];

  const labs =
    labsResult.status === "fulfilled"
      ? mapLabs(
          ensureArray<EHRStudyDTO>(labsResult.value.studies),
          ensureArray<EHRLabDataDTO>(labsResult.value.labData),
        )
      : [];
